...
module.exports.handleCallback = (req, res, next) => {
    ...
    if (!code) return res.redirect('https://moodtunes-rust.vercel.app/index.html?spotify=error');
    ...
    .then(function(r) {
        userModel.saveSpotifyTokens({
            ...
            if (err) return res.redirect('https://moodtunes-rust.vercel.app/index.html?spotify=error');
            res.redirect('https://moodtunes-rust.vercel.app/index.html?spotify=connected');
        });
    })
    .catch(function(e) {
        console.error('Callback error:', e.response ? e.response.data : e.message);
        res.redirect('https://moodtunes-rust.vercel.app/index.html?spotify=error');
    });
};
...
// ── mood genre keywords for search-based recommendations ──────────────────
const MOOD_GENRES = {
    happy:      ['pop', 'feel good', 'upbeat'],
    sad:        ['sad', 'emotional', 'heartfelt'],
    hype:       ['hype', 'trap', 'rap', 'edm'],
    heartbreak: ['heartbreak', 'breakup', 'sad r&b'],
    nostalgic:  ['throwback', '2000s', 'nostalgic'],
    focused:    ['lo-fi', 'instrumental', 'study'],
    chill:      ['chill', 'lo-fi', 'ambient'],
};

// ── recommendations — artist-based search with mood genre diversity ────────
module.exports.getRecommendations = (req, res, next) => {
    ...
    const genres = (mood && MOOD_GENRES[mood]) ? MOOD_GENRES[mood] : [];
    ...
    self.refreshToken(userId, function(refreshErr, freshToken) {
        ...
        function doSearch(token) {
            ...
            // Search by artist name only (never by title — avoids title-matching)
            // Add seed artists for diversity, and mood genre queries
            var queries = [
                // primary artist — search their discography style
                axios.get('https://api.spotify.com/v1/search?q=artist:' + encodeURIComponent(artist) + '&type=track&limit=10', { headers: h }),
            ];

            // seed artists from user's mood playlist
            seeds.slice(0, 3).forEach(function(s) {
                if (s && s.trim()) {
                    queries.push(axios.get('https://api.spotify.com/v1/search?q=artist:' + encodeURIComponent(s.trim()) + '&type=track&limit=8', { headers: h }));
                }
            });

            // mood genre queries for discovery beyond known artists
            genres.slice(0, 2).forEach(function(g) {
                queries.push(axios.get('https://api.spotify.com/v1/search?q=' + encodeURIComponent(g) + '&type=track&limit=8', { headers: h }));
            });
            ...
            .then(function(responses) {
                ...
                responses.forEach(function(r) {
                    (r.data.tracks ? r.data.tracks.items : []).forEach(function(track) {
                        if (!seen[track.id]) {
                            seen[track.id] = true;
                            tracks.push(trackToObj(track));
                        }
                    });
                });
                res.status(200).json({ tracks: shuffle(tracks).slice(0, 30) });
            })
            .catch(function(e) {
